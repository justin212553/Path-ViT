#!/bin/bash
#SBATCH --job-name=PVT-M4-avgpool-ext-eval
...
#SBATCH --output=/pub/wonseukl/Path-ViT/.logs/m4_avgpool_multiseed_external_eval.log

# m4_avgpool_multiseed_kfold_array_hpc.sh(3seed x 5fold=15개 학습)가 저장해 둔 checkpoint
...
#
# 제출: sbatch sbatch/m4_avgpool_multiseed_external_eval_hpc.sh
...
for SEED in "${SEEDS[@]}"; do
  for ((FOLD=0; FOLD<N_FOLDS; FOLD++)); do
    mapfile -t MATCHES < <(ls models/checkpoint/survival_tcga_uni2_seed${SEED}_*STG_R_DISP_COX_ADD_FOLD${FOLD}OF${N_FOLDS}_best_clinical_rna.pt 2>/dev/null | grep AVGPOOL)
