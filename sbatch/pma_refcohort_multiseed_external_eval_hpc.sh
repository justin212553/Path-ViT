#!/bin/bash
#SBATCH --job-name=PVT-PMA-refcohort-ext-eval
...
#SBATCH --output=/pub/wonseukl/Path-ViT/.logs/pma_refcohort_multiseed_external_eval.log

# pma_refcohort_multiseed_kfold_array_hpc.sh(3seed x 5fold=15개 학습)가 저장해 둔 checkpoint
# 15개를 재학습 없이 다시 불러와 --eval-external-ckpt로 external(cptac) 예측만 재추출한다.
# --reference-cohort를 함께 켜야 학습 때와 동일하게 external도 complete-24m 코호트(CPTAC
# 93명 안쪽)로 제한된다 — baseline external(144명 전체)과 직접 비교 시 이 표본 차이를 감안할 것.
#
...
#
# 제출: sbatch sbatch/pma_refcohort_multiseed_external_eval_hpc.sh
...
for SEED in "${SEEDS[@]}"; do
  for ((FOLD=0; FOLD<N_FOLDS; FOLD++)); do
    mapfile -t MATCHES < <(ls models/checkpoint/survival_tcga_uni2_seed${SEED}_*STG_R_REFCOHORT_DISP_COX_ADD_FOLD${FOLD}OF${N_FOLDS}_best_pma.pt 2>/dev/null)
