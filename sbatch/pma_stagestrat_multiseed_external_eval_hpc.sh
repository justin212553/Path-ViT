#!/bin/bash
#SBATCH --job-name=PVT-PMA-stagestrat-ext-eval
...
#SBATCH --output=/pub/wonseukl/Path-ViT/.logs/pma_stagestrat_multiseed_external_eval.log

# pma_stagestrat_multiseed_kfold_array_hpc.sh(3seed x 5fold=15개 학습)가 저장해 둔 checkpoint
# 15개를 재학습 없이 다시 불러와 --eval-external-ckpt로 external(cptac) 예측만 재추출한다.
# external 코호트 자체는 --stage-stratify와 무관(항상 전체 사용)하므로, 이 결과는 기존
# baseline의 external과 직접 비교 가능하다.
#
...
#
# 제출: sbatch sbatch/pma_stagestrat_multiseed_external_eval_hpc.sh
...
for SEED in "${SEEDS[@]}"; do
  for ((FOLD=0; FOLD<N_FOLDS; FOLD++)); do
    mapfile -t MATCHES < <(ls models/checkpoint/survival_tcga_uni2_seed${SEED}_*STG_R_DISP_COX_ADD_STGSTRAT_FOLD${FOLD}OF${N_FOLDS}_best_pma.pt 2>/dev/null)
