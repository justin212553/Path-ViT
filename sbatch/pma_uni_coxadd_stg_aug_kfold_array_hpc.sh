...
log=".logs/train_tcga_seed84_PMA_uni_INT1500_SS_AUX_STG_R_AUG_DISP_COX_ADD_kfold5_fold${FOLD}.log"

echo "=== PMA(uni,cox_add,STG+R,AUG) fold=${FOLD}/5 Start: $(date) (job ${SLURM_JOB_ID}, node $(hostname)) ==="
python -u ./train.py --PMA --rna-genes literature_1500_intersection --dataset tcga --external --seed 84 \
    ...
    --fold "${FOLD}" --n-folds 5 --group-ts 0807pma_uni_coxadd_stg_aug_kfold5_array_seed84 2>&1 | tee "${log}"
echo "=== PMA(uni,cox_add,STG+R,AUG) fold=${FOLD}/5 Complete: $(date) ==="
