#!/bin/bash
#SBATCH --job-name=PVT-M1POOL-uni2-ext-eval
...
#SBATCH --output=/pub/wonseukl/Path-ViT/.logs/m1_pool_uni2_multiseed_external_eval.log

# m1_pool_uni2_multiseed_kfold_array_hpc.sh(3seed x 5fold=15개 학습)가 저장해 둔 checkpoint 15개를
# 재학습 없이 다시 불러와 --eval-external-ckpt로 external(cptac) 예측만 재추출해
# .logs/external_preds/에 CSV로 저장한다. pma_uni2_..._multiseed_external_eval_hpc.sh와 동일한
# 관례(checkpoint는 seed+FOLD{f}OF{n}만 고정하고 나머지는 와일드카드로 찾음).
#
...
#
# 제출(15개 학습 job이 전부 끝나 checkpoint가 다 저장된 뒤에): sbatch sbatch/m1_pool_uni2_multiseed_external_eval_hpc.sh
...
for SEED in "${SEEDS[@]}"; do
  for ((FOLD=0; FOLD<N_FOLDS; FOLD++)); do
    mapfile -t MATCHES < <(ls models/checkpoint/survival_tcga_uni2_seed${SEED}_*FOLD${FOLD}OF${N_FOLDS}*best_m1_pool.pt 2>/dev/null)
