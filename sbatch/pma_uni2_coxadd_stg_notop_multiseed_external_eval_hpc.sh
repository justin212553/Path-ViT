#!/bin/bash
#SBATCH --job-name=PVT-PMA-uni2-notop-ext-eval
...
#SBATCH --output=/pub/wonseukl/Path-ViT/.logs/pma_uni2_coxadd_stg_notop_multiseed_external_eval.log

# pma_uni2_coxadd_stg_notop_multiseed_kfold_array_hpc.sh(3seed x 5fold=15개 학습)가 저장해 둔
# checkpoint 15개를 재학습 없이 다시 불러와 --eval-external-ckpt로 external(cptac) 예측만
# 재추출해 .logs/external_preds/에 CSV로 저장한다.
#
# 2026-08-08에 겪은 사고(글롭이 느슨해 다른 PMA_uni2 변형과 섞여 clinical buffer가 없는
# 체크포인트를 잘못 로드) 재발 방지 — 이 레시피 고유 태그(STG_R_DISP_COX_ADD_NOTOP)까지
# FOLD 앞에 정확히 고정해서 baseline(NOTOP 없는 버전)이나 다른 ablation(NOMEAN/NOATTN/NOSTD/
# TOPFRAC*)과 절대 안 섞이게 한다.
#
...
#
# 제출(15개 학습 job이 전부 끝나 checkpoint가 다 저장된 뒤에): sbatch sbatch/pma_uni2_coxadd_stg_notop_multiseed_external_eval_hpc.sh
...
for SEED in "${SEEDS[@]}"; do
  for ((FOLD=0; FOLD<N_FOLDS; FOLD++)); do
    mapfile -t MATCHES < <(ls models/checkpoint/survival_tcga_uni2_seed${SEED}_*STG_R_DISP_COX_ADD_NOTOP_FOLD${FOLD}OF${N_FOLDS}_best_pma.pt 2>/dev/null)
