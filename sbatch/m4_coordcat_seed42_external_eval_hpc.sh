#!/bin/bash
#SBATCH --job-name=PVT-M4-coordcat-seed42-ext-eval
...
#SBATCH --output=/pub/wonseukl/Path-ViT/.logs/m4_coordcat_seed42_external_eval.log

# m4_coordcat_seed42_kfold_array_hpc.sh(seed42, 5fold 학습)가 저장해 둔 checkpoint 5개를
...
#
# 제출: sbatch sbatch/m4_coordcat_seed42_external_eval_hpc.sh
...
for ((FOLD=0; FOLD<N_FOLDS; FOLD++)); do
  mapfile -t MATCHES < <(ls models/checkpoint/survival_tcga_uni2_seed${SEED}_*M4_uni2_INT1500_SS_AUX_STG_R_DISP_COX_ADD_NOVIT_COORD_CAT_FOLD${FOLD}OF${N_FOLDS}_best_clinical_rna.pt 2>/dev/null)
