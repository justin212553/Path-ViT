...
# (findings_backlog.md, 폴드 간 ~60% 누출) — "가장 잘 나온 레시피 그대로" 재현하려고 일부러
# 안 바꿨다. 절대 수치(internal C)는 두 loss 버전 다 똑같이 낙관적으로 부풀려져 있지만, "loss
...
#
# 완료 후(10개 fold 로그 전부 확인, .logs/kfold_preds/에 CSV 10개):
#   python scripts/pool_multiseed_kfold_preds.py --dataset tcga \
#       --model PORPOISE_uni2native_INT1500_SS_STG_R_DISP_NLLSURV4 --seeds 84 --n-folds 10 --bootstrap 2000
#   (cox 버전과 비교하려면 --model PORPOISE_uni2native_INT1500_SS_STG_R_DISP로 별도 실행)
#
...
log=".logs/train_tcga_seed${SEED}_PORPOISE_uni2native_INT1500_SS_STG_R_DISP_NLLSURV4_kfold10_fold${FOLD}.log"

echo "=== PORPOISE nll_surv loss(uni2native,STG+R,DISP) seed=${SEED} fold=${FOLD}/${N_FOLDS} Start: $(date) (job ${SLURM_JOB_ID}, node $(hostname)) ==="
...
echo "=== PORPOISE nll_surv loss(uni2native,STG+R,DISP) seed=${SEED} fold=${FOLD}/${N_FOLDS} Complete: $(date) ==="
