...
# 튀는 값이 나와 최종 집계에서 제외해온 관례, paper/final_results_summary.md). fold당 인원이
# 너무 작으면 median-split HR 계산이 완전분리로 발산하기 쉬워(실제로 HR이 수천만 단위로 튀는
# 로그 확인) 검정력이 안 나온다 — 5fold(fold당 ~30명)로 되돌리고 시드를 2개로 늘렸다.
# sbatch/porpoise_no_aux_multiseed_kfold_array_hpc.sh와 동일한 SEED_IDX/FOLD 인덱싱 관례
# (3seed 대신 2seed라 --array=0-9는 그대로 유지, 의미만 5fold*2seed로 바뀜).
#
...
log=".logs/train_tcga_seed${SEED}_PMA_uni2native_INT1500_CNV_SS_AUX_STG_R_MUT_NLLSURV4_kfold5_fold${FOLD}.log"
...
python -u ./train.py --PMA --rna-genes literature_1500_intersection --dataset tcga --external --seed "${SEED}" \
    ...
    --fold "${FOLD}" --n-folds "${N_FOLDS}" --group-ts 0906pma_nllsurv_2seed_kfold5_array 2>&1 | tee "${log}"
