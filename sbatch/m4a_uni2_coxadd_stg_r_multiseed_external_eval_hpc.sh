#!/bin/bash
#SBATCH --job-name=PVT-M4A-uni2-coxadd-stg-ext-eval
...
#SBATCH --output=/pub/wonseukl/Path-ViT/.logs/m4a_uni2_coxadd_stg_r_multiseed_external_eval.log

# m4a_uni2_coxadd_stg_r_multiseed_kfold_array_hpc.sh(3seed x 5fold=15개 학습)가 저장한
# checkpoint 15개를 재학습 없이 다시 불러와 --eval-external-ckpt로 external(cptac) 예측만 뽑는다.
# 원리는 pma_uni2_coxadd_stg_multiseed_external_eval_hpc.sh와 동일 — cptac은 어떤 seed/fold
# 조합으로 학습해도 학습 데이터에 안 들어가므로 재학습 없이 forward만 하면 된다.
#
# [체크포인트 glob 주의] 접미사가 PMA(best_pma.pt)와 달리 best_clinical_rna_coattn.pt라
# 헷갈릴 일은 적지만, 관례대로 이 레시피의 정확한 태그(STG_R_DISP_COX_ADD, FOLD 바로 앞)까지
# 고정해서 찾는다.
#
...
#
# 제출: sbatch sbatch/m4a_uni2_coxadd_stg_r_multiseed_external_eval_hpc.sh
...
for SEED in "${SEEDS[@]}"; do
  for ((FOLD=0; FOLD<N_FOLDS; FOLD++)); do
    mapfile -t MATCHES < <(ls models/checkpoint/survival_tcga_uni2_seed${SEED}_*STG_R_DISP_COX_ADD_FOLD${FOLD}OF${N_FOLDS}_best_clinical_rna_coattn.pt 2>/dev/null)
