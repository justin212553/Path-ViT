#!/bin/bash
#SBATCH --job-name=PVT-M4-novit-extraseed-ext-eval
...
#SBATCH --output=/pub/wonseukl/Path-ViT/.logs/m4_novit_extraseed_external_eval.log

# m4_novit_extraseed_kfold_array_hpc.sh(seed 168/210/252, 3seed x 5fold=15개 학습)가 저장해
# 둔 checkpoint 15개를 재학습 없이 다시 불러와 --eval-external-ckpt로 external(cptac) 예측만
# 재추출한다.
#
...
#
# 제출: sbatch sbatch/m4_novit_extraseed_external_eval_hpc.sh
...
for SEED in "${SEEDS[@]}"; do
  for ((FOLD=0; FOLD<N_FOLDS; FOLD++)); do
    mapfile -t MATCHES < <(ls models/checkpoint/survival_tcga_uni2_seed${SEED}_*STG_R_DISP_COX_ADD_NOVIT_FOLD${FOLD}OF${N_FOLDS}_best_clinical_rna.pt 2>/dev/null)
