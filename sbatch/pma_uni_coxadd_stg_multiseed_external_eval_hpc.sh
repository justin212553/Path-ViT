#!/bin/bash
#SBATCH --job-name=PVT-PMA-uni-coxadd-stg-ext-eval
...
#SBATCH --output=/pub/wonseukl/Path-ViT/.logs/pma_uni_coxadd_stg_multiseed_external_eval.log

# pma_uni_coxadd_stg_multiseed_kfold_array_hpc.sh(3seed x 5fold=15개 학습)가 저장한 checkpoint
# 15개를 재학습 없이 다시 불러와 external(cptac) 예측만 뽑는다 — pma_uni2_coxadd_stg_
# multiseed_external_eval_hpc.sh와 동일 원리, backbone만 uni.
#
# [체크포인트 glob 주의] 이 레시피의 정확한 태그(STG_R_DISP_COX_ADD, FOLD 바로 앞)까지 고정해서
# 찾는다 — uni2 버전이나 다른 PMA 변형과 안 섞이게.
#
...
#
# 제출: sbatch sbatch/pma_uni_coxadd_stg_multiseed_external_eval_hpc.sh
...
for SEED in "${SEEDS[@]}"; do
  for ((FOLD=0; FOLD<N_FOLDS; FOLD++)); do
    mapfile -t MATCHES < <(ls models/checkpoint/survival_tcga_uni_seed${SEED}_*STG_R_DISP_COX_ADD_FOLD${FOLD}OF${N_FOLDS}_best_pma.pt 2>/dev/null)
