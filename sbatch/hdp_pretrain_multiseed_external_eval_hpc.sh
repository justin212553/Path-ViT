#!/bin/bash
#SBATCH --job-name=PVT-HDPPRE-ext-eval
...
#SBATCH --output=/pub/wonseukl/Path-ViT/.logs/hdp_pretrain_multiseed_external_eval.log

# hdp_pretrain_multiseed_kfold_array_hpc.sh(2seed x 5fold=10개 학습)가 저장해 둔 checkpoint
...
#       --model HDP_PRETRAIN_INT1500_STG_R --seeds 84,126 --n-folds 5 --bootstrap 2000
#
# 제출(10개 학습 job이 전부 끝나 checkpoint가 다 저장된 뒤에): sbatch sbatch/hdp_pretrain_multiseed_external_eval_hpc.sh
...
for SEED in "${SEEDS[@]}"; do
  for ((FOLD=0; FOLD<N_FOLDS; FOLD++)); do
    mapfile -t MATCHES < <(ls models/checkpoint/survival_tcga_best_hdp_pretrain_int1500_stg_r_fold${FOLD}of${N_FOLDS}_seed${SEED}_light.pt 2>/dev/null)
