...
사용법:
    python -m utils.delete_features                    # cptac+tcga 둘 다 실제 삭제
    python -m utils.delete_features --dataset tcga      # tcga만
    python -m utils.delete_features --dry-run           # 삭제 대상만 출력
"""
import argparse
import sys
...
def main():
    ...
    targets = [p for root in patches_roots for p in sorted(root.rglob(FEATURES_FILENAME))]

    if not targets:
        print(f"삭제할 {FEATURES_FILENAME} 파일이 없습니다: {patches_roots}")
        return
