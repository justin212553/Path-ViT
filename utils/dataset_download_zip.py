...
def main():
    ...
    if failed:
        send_slack(
            f":warning: *CAMELYON17 다운로드 완료 (일부 실패)*\n"
            f"> 완료: {len(done)}명 | 실패: {len(failed)}명 — {[f'patient_{p:03d}' for p in sorted(failed)]}\n"
            f"> 소요 시간: {h}h {m}m {s}s"
        )
    else:
        send_slack(
            f":white_check_mark: *CAMELYON17 다운로드 완료*\n"
            f"> 완료: {len(done)}명 (전체 {len(tasks)}명)\n"
            f"> 소요 시간: {h}h {m}m {s}s"
        )
