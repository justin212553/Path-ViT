#!/bin/bash
#SBATCH --job-name=PVT-M2-aug-kfold
...
#SBATCH --output=/pub/wonseukl/Path-ViT/.logs/train_m2_aug_kfold.log
...
# 백그라운드 프로세스로 동시 실행) — train_m2_kfold_array_hpc.sh(SLURM job array, GPU 5개 필요)
...
# 하나에 5개 동시 실행이 들어간 걸 이미 확인했으니 M2(WSI+Clinical, RNA 없음)도 여유 있게
# 들어갈 것으로 예상.
...
for fold in 0 1 2 3 4; do
    python -u ./train.py --M2 --dataset tcga --external --seed 84 \
        ...
        --fold "$fold" --n-folds 5 "$@" \
        > ".logs/m2_aug_kfold_fold${fold}.log" 2>&1 &
done
wait
echo "=== 5개 fold 전부 종료 ==="
