"""TCGA-BRCA M6(RNAOnly, WSI/Clinical 없음) internal+external 학습/평가 — train_light.py --M6의
BRCA 버전. train_brca_m7.py와 동일 골격, RNA만 남기고 clinical을 뺀 버전.

사용법:
    python -m scripts.train_brca_m6 --seed 84 --gene-selection consistency --fold 0 --n-folds 5
"""
import argparse
import sys
...
from models.rna_only import RNAOnly
...
from scripts.brca_common import (
    BRCACaseDataset, _identity_collate, load_case_table, load_case_table_kfold,
    load_rna_matrix, load_rna_matrix_categorized, load_literature_categories,
...
def main():
    ...
    parser.add_argument("--gene-selection", type=str, default="consistency",
    ...
    rna_df = load_rna_matrix_categorized(literature_categories) if args.gene_selection == "literature_categorized" \
        else load_rna_matrix(gene_ids)
