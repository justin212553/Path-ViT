#!/bin/bash
# M1_uni2_DISP_NOVIT_COORD 5개 checkpoint로 재학습 없이 external(cptac)만 재평가.
set -e
cd "$(dirname "$0")/.."
...
for ((FOLD=0; FOLD<N_FOLDS; FOLD++)); do
    MATCHES=(models/checkpoint/survival_tcga_uni2_seed${SEED}_*M1_uni2_DISP_NOVIT_COORD_FOLD${FOLD}OF${N_FOLDS}_best.pt)
    if [ ! -f "${MATCHES[0]}" ]; then
