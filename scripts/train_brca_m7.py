...
def main():
    ...
    parser.add_argument("--gene-selection", type=str, default="variance",
                         ...
                         help="2026-09-02: 'variance'(기본, scripts/select_brca_rna_genes.py의 "
                              ...
                              "고정 60개를 그대로 씀. 2026-09-04: 'consistency'(Győrffy 2021 "
                              "55-데이터셋 생존분석 + 기존 BRCA 문헌 + PDAC core_driver 재사용, "
                              "882유전자, scripts/select_brca_rna_genes_consistency.py — 우리 "
                              "코호트/라벨 완전 미참조, PDAC의 pdac_consistency_1500과 동일 철학) 추가.")
    ...
    if args.gene_selection == "literature_categorized":
        ...
        gene_ids = None  # 카테고리 평균 경로는 gene_path/gene_ids를 안 씀 — 아래서 별도 처리.
    ...
    if args.gene_selection == "literature_categorized":
        # 2026-09-03: PAAD pathway8과 정확히 같은 원칙 — PAM50(1)+Oncotype DX(1, 서브카테고리
        # 병합)+pan-cancer 6개 = 8개 카테고리 평균. "PDAC에서 뭉치기로 했으면 BRCA에서도
        # 뭉치자"(사용자 지시) — PDAC은 코호트가 작아 카테고리 압축이 실제 이득이었는데
        # (findings_backlog.md), BRCA(코호트가 훨씬 큼)에서도 같은 압축을 적용했을 때 M4 유의성이
        # 나오는지가 이번 실험의 핵심 — 나온다면 "PDAC은 코호트가 작아서 신호가 안 난 거지 모델
        # 자체는 건재하다"는 해석의 근거가 된다.
    ...
    # 2026-09-07: train.py::_FINALEPOCH_ 저장과 동일 관례(사용자 지적) — best-val 체크포인트를
    # 리로드하기 *전에* 마지막 epoch 가중치 그대로 평가한 예측도 별도 CSV로 남긴다
    # (scripts/pool_multiseed_kfold_preds.py --include-final-epoch로 checkpoint 앙상블 가능).
    final_train_metrics = evaluate(model, train_eval_loader, device)
    ...
    if external_loader is not None:
        # 2026-09-10: train_brca_m4.py와 동일한 gap(internal만 FINALEPOCH 저장, external은 빠짐) —
        # paper/brca_final 파일 개수 확인 중 발견(사용자 지적).
