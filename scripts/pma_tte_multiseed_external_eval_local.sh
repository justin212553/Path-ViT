#!/bin/bash
# PMA_uni2_INT1500_SS_AUX_STG_R_DISP_COX_ADD_TTE 15개 checkpoint로 재학습 없이 external(cptac)만 재평가.
set -e
cd "$(dirname "$0")/.."
...
for SEED in "${SEEDS[@]}"; do
  for ((FOLD=0; FOLD<N_FOLDS; FOLD++)); do
    MATCHES=(models/checkpoint/survival_tcga_uni2_seed${SEED}_*STG_R_DISP_COX_ADD_TTE_FOLD${FOLD}OF${N_FOLDS}_best_pma.pt)
    if [ ! -f "${MATCHES[0]}" ]; then
