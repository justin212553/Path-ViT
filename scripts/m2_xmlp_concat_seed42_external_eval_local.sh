#!/bin/bash
# M2_uni2_STG_R_DISP_NOVIT_XMLP (concat 모드) 5개 checkpoint로 재학습 없이 external(cptac)만 재평가.
set -e
cd "$(dirname "$0")/.."
...
for ((FOLD=0; FOLD<N_FOLDS; FOLD++)); do
    MATCHES=(models/checkpoint/survival_tcga_uni2_seed${SEED}_*M2_uni2_STG_R_DISP_NOVIT_XMLP_FOLD${FOLD}OF${N_FOLDS}_best_clinical.pt)
    if [ ! -f "${MATCHES[0]}" ]; then
