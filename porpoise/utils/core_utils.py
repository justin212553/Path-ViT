...
# 2026-09-03: sksurv(scikit-survival)의 ecos 의존성이 이 Windows 환경에서 빌드가 안 돼(Visual
# Studio 빌드툴 필요) 설치 자체가 실패한다 — 원 코드의 모델/학습 로직은 안 건드리고, 동일한
# 통계량(Harrell's C-index)을 계산하는 lifelines로 순수 치환한다. sksurv의 estimate는 "위험도
# 점수"(클수록 고위험) 관례라 lifelines(클수록 좋은 예후로 해석)에 맞추려면 부호를 뒤집어야
# 한다(이 프로젝트의 다른 모든 c-index 계산도 동일 관례로 -risk를 넘김).
def concordance_index_censored(event_indicator, event_time, estimate, tied_tol=1e-8):
    c = concordance_index(event_time, -np.asarray(estimate), np.asarray(event_indicator))
    return (c, None, None, None, None)

import torch

from porpoise_datasets.dataset_generic import save_splits
...
from models.model_porpoise import PorpoiseMMF, PorpoiseAMIL  # 2026-09-03: PorpoiseMMF_Fast는 이 저장소에 아예 정의돼 있지 않음(원본 코드 자체의 미완성/누락) — 안 쓰이는 import라 제거
...
def train(datasets: tuple, cur: int, args: Namespace):
    ...
    if args.model_type == 'porpoise_mmf':
        ...
        model_dict = {'n_classes': args.n_classes, 'path_input_dim': args.path_input_dim}
...
def summary_survival(model, loader, n_classes):
    ...
    for batch_idx, (data_WSI, data_omic, y_disc, event_time, censor) in enumerate(loader):
        ...
        event_time = (event_time.item() if hasattr(event_time, "item") else event_time)
        censor = (censor.item() if hasattr(censor, "item") else censor)
        all_risk_scores[batch_idx] = risk
