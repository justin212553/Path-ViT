...
# 2026-09-05: main.py와 동일한 이유로 주석 처리(--mode graph/cluster 전용, path 모드에선
# Batch 클래스가 실제로 안 쓰임 — 파일 안에서 참조되는 곳이 없음을 확인했음).
...
def collate_MIL_survival(batch):
    # 2026-09-03: torch 1.6 시절엔 float 텐서 리스트를 torch.LongTensor(...)/torch.FloatTensor(...)에
    # 그대로 넣으면 암묵적으로 스칼라 변환됐지만, 최신 torch(2.x)는 __index__에 정수 dtype 텐서만
    # 허용하도록 바뀌어 TypeError가 난다 — 모델 로직은 그대로 두고 .item()으로 명시적 스칼라 추출만 추가.
    img = torch.cat([item[0] for item in batch], dim = 0)
