...
import numpy as np
# 2026-09-05: torch_geometric는 --mode graph/cluster 전용인데 이 파일 맨 위에서 무조건
# import해서, 우리 환경에 안 깔려 있으면(--mode path만 쓸 건데도) 실행 자체가 안 된다.
# 알고리즘/모델/손실함수는 전혀 안 건드리고, 순수 환경 호환성 때문에 주석 처리만 한다
# (원본 PORPOISE 코드의 이 한 줄만 예외 — 나머지는 전부 원본 그대로).
...
### Internal Imports
from porpoise_datasets.dataset_survival import Generic_WSI_Survival_Dataset, Generic_MIL_Survival_Dataset
