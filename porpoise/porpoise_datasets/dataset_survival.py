...
class Generic_WSI_Survival_Dataset(Dataset):
    def __init__(self,
        ...
        # 2026-09-03: 원래 코드는 "마지막 컬럼(disc_label)을 맨 앞으로 옮기고 처음 12개를
        # metadata로 가정 + 정확한 순서까지 assert"하는 위치 기반 로직이었는데, 저장소가 실제로
        # 배포한 예제 CSV(datasets_csv/tcga_brca_all_clean.csv.zip)조차 이 assert를 통과하지
        # 못한다("Unnamed: 0" 컬럼 자체가 없음 — 직접 pd.read_csv로 확인) — 저장소 자체의
        # 코드/데이터 불일치로 판단, 이름 기반으로 명시적으로 재구성하도록 고쳤다(의도는
        # 그대로 유지: metadata 컬럼들을 앞으로 모으고 나머지 전부를 유전체 feature로 취급).
        metadata_candidates = ['disc_label', 'Unnamed: 0', 'case_id', 'label', 'slide_id',
                                'age', 'site', 'survival_months', 'censorship', 'is_female',
                                'oncotree_code', 'train']
        metadata = [c for c in metadata_candidates if c in slide_data.columns]
        other_cols = [c for c in slide_data.columns if c not in metadata]
        slide_data = slide_data[metadata + other_cols]
