...
class PorpoiseAMIL(nn.Module):
    def __init__(self, size_arg = "small", n_classes=4, path_input_dim=1024):
        # 2026-09-03: path_input_dim 파라미터화 -- 원래 1024(ResNet50 CLAM feature) 고정이었는데
        # UNI2 feature(1536차원) 등 다른 patch encoder를 쓰려면 필요(PorpoiseMMF는 이미 지원했음).
