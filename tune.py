...
# SLURM이 잡아주는 노드 로컬 스크래치($TMPDIR, 없으면 /tmp)를 사용해 이 지연을 없앤다.
# 주의: 이 경로는 보통 SLURM job이 끝나면 지워지므로, 결과를 보존하려면
# job 종료 전 STORAGE_PATH를 영구 저장소(dfs6b 등)로 복사해야 한다.
STORAGE_PATH = str(Path(os.environ.get("TMPDIR", "/tmp")) / "path_vit_ray_results")
