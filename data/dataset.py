...
한 dataset(tcga/cptac)당 하나의 코호트 전체를 반환한다 — 코호트 내부 train/val split은
하지 않는다. 검증은 train.py에서 다른 코호트로 cross-dataset 방식으로 수행한다
(예: tcga로 학습 → cptac으로 검증, 반대 방향도 별도 실행으로 수행).

사용법 예:
    from config import DataConfig
    from data.dataset import WSISurvivalDataset
    train_ds = WSISurvivalDataset(DataConfig(), dataset="tcga")
    val_ds   = WSISurvivalDataset(DataConfig(), dataset="cptac")
"""
...
class WSISurvivalDataset(Dataset):
    ...
    def __init__(
        ...
        self.items  = merged[has_patches].reset_index(drop=True)
        if self.items.empty:
            raise RuntimeError(
